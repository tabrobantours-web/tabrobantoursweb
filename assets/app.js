...
(() => {
  ...
  if(window.gsap && window.ScrollTrigger && !matchMedia('(prefers-reduced-motion: reduce)').matches){
    ...
    if(handpicked && handpickedTrack){
      ...
      ScrollTrigger.matchMedia({
        ...
        "(max-width: 740px)":()=>setupHorizontal(true)
      });
    }
    ...
    if(fleetSection){
      ...
      ScrollTrigger.matchMedia({
        ...
        "(max-width: 740px)":()=>setupFleet(true)
      });
    }
  ...
  if(sf){
    ...
    form.addEventListener('submit',e=>{e.preventDefault();toast('Prototype submitted — connect this form to your CRM or email service for production.');form.reset();step=1;render()});
  }

  $$('[data-demo-form]').forEach(form=>form.addEventListener('submit',e=>{e.preventDefault();toast('Prototype form submitted — backend integration is ready to be connected.');form.reset()}));
